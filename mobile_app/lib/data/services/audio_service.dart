...
class AudioService {
  ...
  /// Warm up the Web Audio Context (HTML5 Autoplay bypass hack)
  /// Must be called synchronously inside a user click gesture (e.g. mic button tap)
  Future<void> warmUpWebAudio() async {
    if (!kIsWeb) return;
    try {
      // 44-byte minimal valid WAV header containing 0 samples of silence
      ...
      final source = await AudioPlayerHelper.getAudioSource(silentWav, 'audio/wav');
      ...
      debugPrint('[IST-RVT] Browser Web Audio Context successfully unlocked 🔓');
    } catch (e) {
      debugPrint('[IST-RVT] Audio context warm-up warning: $e');
    }
  }
